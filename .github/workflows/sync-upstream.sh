...
verify() {
  ...
  # Quiet while passing, but print everything on failure. Swallowing this
  # output leaves a red CI run with no way to tell what broke.
  local out
  out="$(mktemp)"
  if go test ./... >"$out" 2>&1; then
    echo "  go test OK    ."
  else
    warn "go test failed in the root module:"
    grep -vE '^(ok|\?)' "$out" || true
    die "go test failed in the root module"
  fi
}
